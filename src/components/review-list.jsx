...
class ReviewList extends Component {
  ...
  render() {
    ...
    const {showAllReviews, morePressedCount, showCSVModal} = this.state;
    ...
    if (cards) {
      function generateCSV() {
        ...
        const data = cards.map(({repoOwner, repoName, number}) => {
          ...
          if (card.review) {
            const ret = [repoOwner, repoName, number, card.review.updatedAt, card.review.user.login, card.review.title];
    ...
    let newReviewURL = primaryRepo &&
      `${rootURL}${primaryRepo.repoOwner}/${primaryRepo.repoName}/Reviews/new`;
    if (primaryRepo && label && label.name !== UNCATEGORIZED_NAME) {
      newReviewURL = `${newReviewURL}?labels=${encodeURIComponent(label.name)}`;
    }

    const header = (
      <h2 className={className}>
        {iconEl}Reviews in Need ({countOrDownloadLink})
      </h2>
    );
...
export default DropTarget(ItemTypes.CARD, cardListTarget, collect)(ReviewList);
